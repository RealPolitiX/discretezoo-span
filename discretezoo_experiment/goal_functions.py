...
class GoalFunction(ReprMixin, ABC):
    ...
    #    inputs = [att.tokenizer_input for att in attacked_text_list]
    #    outputs = [self.model(tkinp) for tkinp in inputs]
    #    return outputs
...
class ClassificationGoalFunction(GoalFunction):
  ...
  def _process_model_outputs(self, inputs, scores):
    ...
    if not isinstance(scores, torch.Tensor):
      raise TypeError("Must have list, np.ndarray, or torch.Tensor of "
